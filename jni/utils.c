...
#include <sys/stat.h>
#include <unistd.h>
// For opendir(), readdir(), closedir(), DIR, struct dirent.
#include <dirent.h>
...
#ifndef DEF_BUF
	// Raised from 256 so we can add headers to 255-256 byte buffers
	#define DEF_BUF 512
#endif

// 8 prepended bytes + 255 max bytes HCI data/parameters + 1 trailing byte to align
#define MAX_HCI 264

// Does not work: "/data/data/fm.a2d.sf/files/g2_audio_done", "/dev/g2_audio_done";
char * g2_audio_done_file = "/sdcard/g2_audio_done";
...
// Int 2^32 need max 10 characters, 2^64 need 21
#define MAX_ITOA_SIZE 32
...
char* user_char_dev_get(char* dir_or_dev, int user) {
	...
	// For all files/dirs in this directory... (Could terminate with errno set !!)
...
// LG or LGE
int lg_get() {
	return !strncmp(sys_prop_manuf,"LG", strlen("LG"));
}
...
int qcv_need_internal_antenna_get () {
	...
	if (
		!strncmp(sys_prop_device, "C2", strlen("C2")) ||
...
int file_find(char* dir, char* pat, char* path_buf, int path_len) {
	...
	// Routine is recursive; if more than 16 subdirectories nested... (/system/xbin/bb -> /system/xbin)
	if (nest > 16) {
	...
	while ((dirp = readdir (dp)) != NULL) {
		...
		if (S_ISDIR (sb.st_mode)) { // If this is a directory...
			// Recursively call self: Go deeper to find the file, If found...
			if (file_find (filename, pat, path_buf, path_len)) {
				closedir(dp); // Close the directory.
				nest--;
				return 1; // File found
			}
